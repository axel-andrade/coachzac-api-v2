...
function Analyze(request, response) {
    ...
    var _super = {
        ...
        recalculatePlayer: function (object) {
            ...
            console.log("Player Points", analyze.get("player").get("points"));
        ...
        publicMethods: {
            ...
            createAnalyze: function () {
                ...
                return Parse.Promise.all(promises).then(function (results) {
                    _player = results[1];
                    _fundament = results[2];
                    let analyze = new Define.Analyze();

                    if (!_player.get("userGroup") || _player.get("userGroup") !== "player" || _currentUser.id != _player.get("coach").id)
                    ...
                    _params.player = _player;
                    _params.fundament = _fundament;
                    ...
                    return analyze.save(_params, { useMasterKey: true });
                     ...
                     let data = analyze.toJSON();
                     data.player = _player.toJSON();
            ...
            editAnalyze: function () {
                ...
                promises.push(utils.verifyCoachPermissions(_currentUser)); //[0] verify coach permissions
                promises.push(query.get(_params.analyzeId)); //[1] get analyze
                return Parse.Promise.all(promises).then(function (results) {
            ...
            getAnalyzesMostRecent: function () {
                ...
                let promises = [];
                let qPlayers = new Parse.Query(Parse.User);
                qPlayers.equalTo("coach",_currentUser);
                ...
                query.matchesQuery("player",qPlayers);
