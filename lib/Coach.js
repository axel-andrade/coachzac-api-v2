    ...
    var _super = {
        ...
        publicMethods: {
            createCoach: function () {
                ...
                let requiredFields = utils.verifyRequiredFields(_params, ["name", "email", "password", "team", "dateOfBirth", "userGroup"]);
                if (requiredFields.length > 0) {
                    _response.error("Field(s) '" + requiredFields + "' are required.");
                }

                if (Define.userGroups.indexOf(_params.userGroup) < 0) {
                    _response.error(Messages.error.ERROR_INVALID_USER_GROUP);
                }
                ...
                user.set('userGroup', _params.userGroup);
