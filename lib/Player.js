        ...
        publicMethods: {
            ...
            deletePlayer: function () {
                ...
                return Parse.Promise.all(promises).then(function (results) {
                    ...
                    promises.push(results[1].destroy({ useMasterKey: true })); //[0] delete player
                    
                    let q1 = new Parse.Query("Analyze");
                    q1.equalTo("player", results[1]);

                    promises.push(q1.find()); // [1] get analyzes of deleted player

                    let q2 = new Parse.Query("Training");
                    q2.equalTo("player", results[1]);
                    promises.push(q2.find());// [2] get trainings of deleted player

                    return Parse.Promise.all(promises);
                }).then(function(results){
                    promises = [];
                    promises.push(Parse.Object.destroyAll(results[1]));
                    promises.push(Parse.Object.destroyAll(results[2]));   
            ...
            getPlayers: function () {
                ...
                return Parse.Promise.when(promises).then(function (results) {
                    ...
                    if (_params.favorited == true || _params.favorited == false)
